...
| **M2** | `progress` de `updateTicketStatus` sigue siendo un `Int` (porcentaje), no la fecha/franja agendada. **Quinta versión seguida (contando solo las versiones cruzadas: la v1.71 y la v1.72 no) sin estructura de `details` para `STARTED`/`PROGRESS`** (§8.2 de la v1.73, "details obligatorio: Ninguno"). Hay que reclamarlo explícitamente en vez de esperar la próxima versión — aviso redactado abajo | 🔴 Bloqueante | 19 sep 2026 |
...
| **M2** | **v1.73 (WIP) cruzada: no cambia lo que implementamos.** Reemplaza a la v1.72 WIP; la v1.71 y la v1.72 nunca se cruzaron y no tenemos el texto de la v1.70, así que no se puede aislar qué cambió entre la 1.70 y la 1.73. Coinciden el sobre (§4), `updateTicketStatus` (§8), `ticketUpdated` (§7) y §12. Detalle en [`ticketUpdated`](eventos/consumidos/ticketUpdated.md) y [`updateTicketStatus`](eventos/publicados/updateTicketStatus.md) | ✅ Adoptado (Issue #167) | 19 sep 2026 |
| **M2** | **`responsibleAreaId === 'M6'` en `TicketsConsumer` es correcto.** Los ejemplos de la v1.73 usan `"M6"` en `ticketCreated`, `ROUTED` e `INFORMATION_PROVIDED`, y §4/§5.1 lo definen como identificador canónico `Mx` del módulo/área. Cierra la duda de la auditoría del 18/09 | ✅ Confirmado | 19 sep 2026 |
...
| **M2** | Confirmar que `occurredAt` del sobre viaja siempre en ISO 8601 (con `Z` o con offset). Nuestro inbox lo valida estricto desde el Issue #182 y responde 400 si no lo es. Ningún documento de la cohorte fija el formato | ⚠️ Aviso pendiente | 20 sep 2026 |
...
> Cruzamos la v1.73 y no cambia nada de lo que implementamos. Ya teníamos adoptada la v1.70 completa: `publicId` ya viaja en nuestro modelo como referencia humana y respetamos que no es credencial (no lo usamos para autorizar ni lo aceptamos en el seguimiento público). Nos queda un pedido que ya va por la quinta versión (v1.73): **necesitamos mandarles la fecha y la franja horaria agendadas de un servicio**, y hoy no hay campo. `progress` es un `Int` de porcentaje y §8.2 de la v1.73 mantiene "details obligatorio: Ninguno" para `STARTED`/`PROGRESS`. ¿Lo mandamos como texto dentro de `publicMessage`, o definen algo tipo `details.schedule { scheduledDate, windowFrom, windowTo }`? Con cualquiera de las dos respuestas cerramos el tema; lo que no podemos es seguir sin lugar donde ponerlo.

> Una consulta sobre el sobre común (§4): **¿`occurredAt` viaja siempre en ISO 8601, con `Z` o con offset?** Por ejemplo `2026-09-18T10:00:00Z` o `2026-09-18T07:00:00-03:00`. Nuestro inbox pasó a validarlo de forma estricta y responde 400 si el valor no es una fecha ISO 8601 válida (por ejemplo `"2026-09-18 10:00:00"` con espacio, `"18/09/2026"` o un número epoch); antes lo aceptaba como texto libre. Necesitamos confirmar que su emisor no manda otro formato, porque un 400 de nuestro lado puede llevarlos a reintentar o a descartar el evento. Si mandan otro, avísennos cuál y lo aceptamos.
...
**Actualización 20/09 — formato de `occurredAt`.** Desde el Issue #182 nuestro inbox valida `occurredAt` como ISO 8601 estricto (con `Z`, con offset o con milisegundos) y responde 400 si no lo es; antes lo aceptaba como texto libre. Ningún documento de la cohorte fija el formato, ni la v1.6 ni la v1.73 (§4 solo lo lista como campo del sobre). Todo lo que emitimos nosotros sale de `toISOString()`, así que es compatible; lo que no sabemos es qué manda M2. Un 400 nuestro puede hacer que reintenten o descarten el evento, por eso está el aviso pendiente de abajo.

**Actualización 19/09 — v1.73 (WIP).** Reemplaza a la v1.72 WIP; la v1.71 y la v1.72 nunca se cruzaron. Cruzada contra el código, no hay cambios: sobre (§4), `updateTicketStatus` (§8), `ticketUpdated` (§7) y §12 coinciden. Confirma que `responsibleAreaId` es el identificador canónico `Mx` (los ejemplos usan `"M6"`), así que nuestro filtro `responsibleAreaId === 'M6'` es correcto. **El bloqueante de la fecha agendada sigue igual: quinta versión seguida** (§8.2, "details obligatorio: Ninguno").

...
export class TreesController {
  ...
  @ApiOperation({
    ...
    description:
      'Da de alta un árbol en el inventario de arbolado urbano con su código de relevamiento (único), especie, ubicación y datos dendrométricos.',
  })
  ...
  @ApiResponse({
    status: 403,
    description: 'Sin permisos para registrar árboles',
  ...
  @ApiResponse({
    status: 409,
    description: 'Ya existe un árbol con ese código de relevamiento',
    type: ErrorResponseDto,
  })
  @ApiResponse({ status: 500, description: 'Error interno del servidor', type: ErrorResponseDto })
  async create(@Body() dto: CreateTreeDto): Promise<TreeResponseDto> {
    return this.treesService.create(dto);
  }

  @Get()
  @ApiOperation({
    summary: 'Listar árboles del inventario',
    description:
  ...
  @ApiOperation({
    ...
    description: 'Retorna el detalle completo de un árbol del inventario.',
  })
  ...
  @ApiResponse({ status: 500, description: 'Error interno del servidor', type: ErrorResponseDto })
  async findOne(@Param('id', ParseUUIDPipe) id: string): Promise<TreeResponseDto> {
    return this.treesService.findOne(id);
  }

  @Patch(':id')
  @ApiOperation({
    summary: 'Actualizar datos de un árbol',
    description: 'Actualiza los campos mutables. El código de relevamiento no se puede cambiar.',
  })
  @ApiParam({ name: 'id', description: 'UUID del árbol', format: 'uuid' })
  @ApiResponse({
    status: 200,
    description: 'Árbol actualizado exitosamente',
  ...
  @ApiOperation({
    summary: 'Desactivar un árbol del inventario',
    description: 'Soft-delete: marca el árbol como inactivo. No elimina el registro.',
  })
  @ApiParam({ name: 'id', description: 'UUID del árbol', format: 'uuid' })
